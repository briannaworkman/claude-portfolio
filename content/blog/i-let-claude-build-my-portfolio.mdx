...
🎨 **Design taste.** The dark color palette, the violet accent color, the decision to do a "premium, not cluttered" aesthetic were my calls. The AI executed them, but didn't invent them.
...
The latest release is [v1.1.0](https://github.com/briannaworkman/claude-portfolio/releases/tag/v1.1.0), which added an icon system (GitHub, LinkedIn, and external link SVG components) built through the same brainstorm → spec → plan → ship workflow — proof the process works for incremental features, not just greenfield builds.
